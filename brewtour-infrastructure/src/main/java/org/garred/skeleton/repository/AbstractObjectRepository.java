...
import org.garred.skeleton.api.Identifier;
...
public abstract class AbstractObjectRepository<I extends Identifier,T> implements Repository<I,T> {
	...
	public void update(I key, T value) {
		...
		try {
			...
			throw new ObjectDoesNotExistException(SkelAggregate.class, key.getId(), e);
		}
		...
		if(result != 1) {
			throw new OptimisticLockingException(SkelAggregate.class, key.getId());
		}
	}
	...
	private T deserialize(Reader reader) {
		try {
